...
import { usersTable, referralsTable, tasksTable, withdrawalsTable } from "@workspace/db";
import {
...
router.get("/stats", async (_req, res): Promise<void> => {
  ...
  const [activeTaskCount] = await db.select({ count: count() }).from(tasksTable).where(eq(tasksTable.isActive, true));
  const [pendingW] = await db.select({ count: count() }).from(withdrawalsTable).where(eq(withdrawalsTable.status, "pending"));
  const [approvedW] = await db.select({ total: sum(withdrawalsTable.amount) }).from(withdrawalsTable).where(eq(withdrawalsTable.status, "approved"));
  ...
  res.json({
    ...
    totalWithdrawn: Number(approvedW?.total ?? 0),
  });
});
...
router.patch("/admin/tasks/:taskId", async (req, res): Promise<void> => {
  ...
  if (isNaN(taskId)) { res.status(400).json({ error: "ID invalide" }); return; }
